	...
	private void watchForFsChanges(Watcher watcher) {
		...
		try {
			...
			for (String fs : fsChildren) {
				...
				System.out.println("the identifier1 is " + datajson.getString("id"));
	...
	public void ReadConfigurationFile() {
		...
		//Read configuration file
	        try {
	            ...
	            instance.secretkey=classElement.getChild("skkey").getValue();
	...
	public void PublishService(ServletContextEvent sce) {
		...
		try {
	       ...
	       try {
			...
			System.out.println("The identifier2 is " + instance.identifier);
			instance.zoo.create(dirpath+"/"+instance.identifier, configJSON.toString().getBytes(),Arrays.asList(acl),
